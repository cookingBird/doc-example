...
(function(global, factory) {
	...
	function d3_contours() {
		...
		contours.thresholds = function(_) {
			return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant(slice.call(_)) :
				constant(_), contours) : threshold;
		};
	...
	// 生成克里金grid
	kriging.getGridInfo2 = function(bbox, variogram, width) {
		var grid = [];
		//x方向
		var xlim = [bbox[0], bbox[2]];
		var ylim = [bbox[1], bbox[3]];
		var zlim = [variogram.t.min(), variogram.t.max()];
		var range=variogram.t.max()-variogram.t.min();
		...
		for (let j = 0; j < y_width; j++) {
			for (let k = 0; k < x_width; k++) {
				xtarget = bbox[0] + k * x_resolution;
				ytarget = bbox[1] + j * y_resolution;
				//ztarget=(kriging.predict(xtarget, ytarget, variogram)-zlim[0])/range;
				ztarget=kriging.predict(xtarget, ytarget, variogram);
				if(ztarget<=0.0)
				{
					continue;
				}
				...
				// grid.push(kriging.predict(xtarget, ytarget, variogram));
				grid.push({x:xtarget,y:ytarget,value:ztarget});
			}
		}
	...
	function _getKrigingGridInfo(featureCollection, weight, krigingParams) {
		...
		let gridinfo = kriging.getGridInfo(extent, variogram, 650);
		return gridinfo;
	}
	...
	*/
	...
	function getKrigingsPoint(featureCollection, weight, krigingParams) {
		let values = [],
			lons = [],
			lats = [];
		let extent = [100000000, 100000000, -100000000, -100000000];
		...
		let variogram = kriging.train(values, lons, lats, krigingParams.model, krigingParams.sigma2, krigingParams.alpha);
		let gridinfo = kriging.getGridInfo2(extent, variogram, 300);
		return gridinfo;
		
	}
	exports.drawCanvasContour = drawCanvasContour;
	exports.getVectorContour = getVectorContour;
	exports.getKrigingsPoint = getKrigingsPoint;
